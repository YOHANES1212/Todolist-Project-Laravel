        ...
        <div class="min-h-screen flex items-center justify-center px-4 py-10">
            <div class="relative w-full max-w-[480px] overflow-hidden rounded-[2rem] bg-white/95 shadow-[0_45px_120px_rgba(0,0,0,0.18)] backdrop-blur-sm ring-1 ring-white/60">
                ...
                <div class="relative flex flex-col gap-6 px-8 py-12 sm:px-12">
                    <div class="space-y-3">
                        <h1 class="text-3xl font-semibold tracking-tight text-slate-900">Lupa Password?</h1>
                        <p class="max-w-sm text-sm text-slate-500">Masukkan email akun kamu. Kami akan mengirim link untuk reset password.</p>
                    </div>
                    ...
                    <form action="{{ route('password.email') }}" method="POST" class="space-y-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input id="email" name="email" type="email" value="{{ old('email') }}" placeholder="Enter Email" required autofocus class="w-full rounded-2xl border border-slate-200 bg-[#F8F8F8] py-3 pl-12 pr-4 text-sm text-slate-900 outline-none transition focus:border-[#ff5d5a] focus:ring-2 focus:ring-[#ff5d5a]/15" />
                            </div>
                        </div>

                        <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl bg-[#ff5d5a] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#ff4340] focus:outline-none focus:ring-2 focus:ring-[#ff5d5a]/50">Kirim Link Reset</button>
                    </form>
